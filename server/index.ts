...
(async () => {
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    // On boot, run yesterday's payout if it hasn't been processed yet.
    // Handles the case where the server restarted after midnight PST and the sweep timer reset.
    // The dedup check in processPayoutForPeriod prevents any double-payout.
    setTimeout(async () => {
      try {
        ...
        for (const item of schedule) {
          const result = await processPayoutForPeriod(
            item.period, item.periodLabel, item.periodStart, item.periodEnd,
            (msg: string) => console.log(`[startup-payout] ${msg}`)
          );
    ...
    }, 45_000); // 45s after boot — lets sports sync and grading finish first
  }
